...
function linearQuery(
  ...
  const result = execSync(
    `curl -s -X POST "${API}" -H "Authorization: ${authHeader}" -H "Content-Type: application/json" -d '${body.replace(/'/g, "'\\''")}'`,
...
function* linearPaginate(ctx: QueryContext, query: string, dataPath: string[]) {
  let after: string | null = null;
  while (true) {
    const vars: any = { first: 100 };
...
export default function linear(dl: DriplinePluginAPI) {
  dl.setName("linear");
  dl.setVersion("0.1.0");
      ...
      for (const i of linearPaginate(ctx, query, ["issues"])) {
        yield {
          ...
          labels: JSON.stringify(
            i.labels?.nodes?.map((l: any) => l.name) || [],
          ),
    ...
    *list(ctx) {
      ...
      for (const p of linearPaginate(ctx, query, ["projects"])) {
        yield {
          ...
          team_names: JSON.stringify(
            p.teams?.nodes?.map((t: any) => t.name) || [],
          ),
