...
function getDriplineTables(cwd: string): TableSchema[] | null {
  try {
    const output = execSync("npx tsx src/main.ts tables --json", {
      encoding: "utf-8",
      timeout: 30000,
      cwd,
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    if (!alreadyInjected) {
      ...
      ctx.sessionManager.appendCustomMessageEntry(
        "dripline-context",
        "## Dripline tables\n" +
          `You have access to dripline (SQL query tool) with ${tableCount} tables from ${pluginCount} plugins. ` +
          "Use `dripline query \"SQL\"` to query any of these tables. " +
          "Use `dripline query \"SQL\" -o json` for JSON output.\n\n" +
          schema,
        true,
      );
    }
